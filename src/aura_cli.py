...
from aura_helper import AuraHelper, AuraError
...
def audit(url, cookies, object_list, proxy, fetch_max_data=False, insecure=False, app=None, aura_path="/aura", context=None, token="null", no_gql=False, ignore_list=None):
	...
	if object_list:
		...
		if valid_objects:
			...
			raise AuraError('No valid objects provided with -l')
		if invalid_objects:
			logger.warning(f'Ignoring invalid objects: {",".join(invalid_objects)}')
	if objects is None:
		raise AuraError('Could not find any objects')

	# Filter out ignored objects
	if ignore_list:
		before_count = len(objects)
		objects = [obj for obj in objects if obj.lower() not in ignore_list]
		filtered_count = before_count - len(objects)
		if filtered_count > 0:
			logger.info(f'Filtered out {filtered_count} ignored objects, {len(objects)} remaining')
		if len(objects) == 0:
			raise AuraError('All objects were filtered by ignore list')
	...
	return {
		"url": url,
		"records": all_records,
		"gql_records": all_records_gql,
		"gql_enabled": aura.gql_enabled,
		"recordlists": recordlists,
		"home_urls": home_urls,
		"csp_trusted": aura.csp_trusted,
		"custom_controllers": custom_controllers,
	}


def save_results(result, output_dir):
	write_records_to_directory(result["records"], output_dir, "records")
	write_records_to_directory(result["gql_records"], output_dir, "gql_records")
	write_misc_to_directory(result["recordlists"], output_dir, sub_dir='misc', file_name='recordlists.json')
	write_misc_to_directory(result["home_urls"], output_dir, sub_dir='misc', file_name='homeurls.json')
	write_misc_to_directory(result["csp_trusted"], output_dir, sub_dir='misc', file_name='csp_trusted_sites.json')
	write_misc_to_directory(result["custom_controllers"], output_dir, sub_dir='misc', file_name='custom_controllers.json')
...
def print_summary(result):
	print('')
	print('--- Summary ---')
	print(draw_table(result["records"]))
	print('')
	if result["gql_enabled"]:
		print('--- Summary GraphQL ---')
		print(draw_table(result["gql_records"]))
		print('')
...
/_/   \_\__,_|_|  \__,_|___|_| |_|___/ .__/ \___|\___|\__\___/|_|
	...
	ignore_list = None
	if args.ignore_list:
		from batch_scanner import load_ignore_list
		ignore_list = load_ignore_list(args.ignore_list)
		logger.info(f'Loaded {len(ignore_list)} objects to ignore')

	# Batch mode
	if args.batch_file:
		if args.url or args.aura_request_file:
			logger.error('Cannot use --batch-file with -u or -r. Use --batch-file alone.')
			exit()
	...
	if args.aura_request_file:
		...
		if url is None:
			logger.error('Specify a URL, a request file, or a batch file')
			exit()
